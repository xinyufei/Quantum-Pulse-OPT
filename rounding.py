...
def rounding(file_name, type, min_up_times=0):
    ...
    ax1.step(t[:-1] / ts_interval, b_rel[:, 0], label="b_rel", color="C0", linestyle="dashed", where="post")
    ax1.step(t[:-1] / ts_interval, b_bin[0, :], label="b_bin", color="C0", where="post")
    ax1.legend(loc="upper left")
    ax1.set_ylabel("u_1")
    ax2.step(t[:-1] / ts_interval, b_rel[:, 1], label="b_rel", color="C1", linestyle="dashed", where="post")
    ax2.step(t[:-1] / ts_interval, b_bin[1, :], label="b_bin", color="C1", where="post")
    ax2.legend(loc="upper left")
    ax2.set_ylabel("u_2")
    plt.savefig(file_name.split(".csv")[0] + "_bvsr_" + str(min_up_times) + ".png")
